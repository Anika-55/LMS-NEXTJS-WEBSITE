...
//               <CourseCard
//                 key={course.slug?.current ?? course.id}
//                 slug={{ current: course.slug?.current ?? course.id }}
//                 title={course.title}
...
export default async function MyCoursesPage() {
  ...
  // Filter courses where user has completed at least one lesson
  const startedCourses = courses
    .map((course) => {
      ...
      const completedLessons = course.modules.reduce(
        (acc, m) =>
          acc +
          (m.lessons?.filter((l) => l.completedBy?.includes(userId)).length ?? 0),
        0
      );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      {startedCourses.length === 0 ? (
        <p className="text-zinc-400 mt-4">You haven't started any courses yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {startedCourses.map((course) => (
            <CourseCard
