...
// Local version: update course completion in memory
const toggleCourseCompletionLocal = async (
  courseId: string,
  currentState: boolean
) => {
  // Normally you would call an API. Here we just return a success object.
  return new Promise<{ success: boolean; isCompleted: boolean }>((resolve) => {
    setTimeout(() => {
      resolve({ success: true, isCompleted: !currentState });
    }, 300); // simulate delay
  });
};
...
export function CourseCompleteButton({
  ...
  const handleToggle = () => {
    startTransition(async () => {
      const result = await toggleCourseCompletionLocal(courseId, isCompleted);
      if (result.success) {
